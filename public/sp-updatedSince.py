...
def get_data(url):
    try:
        ...
        if int(conn.code) != httplib.OK:
            sys.stderr.write("Error getting data. Some data may already have been retieved.\n")
...
def output_csv_header_line(outfile):
...
def output_csv(data, location_ids, outfile):
    writer = UnicodeWriter(outfile)
    for record in data:
...
def load_updated_locations(options):
    ...
    output_csv_header_line(options.outfile)
    while keep_going:
        ...
        # Output the data
        output_csv(response['results'], output_location_ids, options.outfile)
