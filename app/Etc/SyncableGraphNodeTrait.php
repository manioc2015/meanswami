...
trait SyncableGraphNodeTrait
{
    ...
    public static function createOrUpdateGraphNode($data)
    {
        ...
        if ($data instanceof GraphObject || $data instanceof GraphNode || $data instanceof GraphUser) {
        ...
        $attribute_fbid = [static::getGraphNodeKeyName() => $data['id']];
        $email = nonblank_or($data['email'], '');
        $attribute_email = ['email' => $email];
